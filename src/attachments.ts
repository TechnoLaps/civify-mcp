...
export function attachmentUrl(value: string): URL {
  let url: URL;
  try { url = new URL(value); } catch { throw new Error("ATTACHMENT_UNAVAILABLE: Provide a real HTTPS attachment download URL or resume_text."); }
...
export const publicLookup: NonNullable<https.AgentOptions["lookup"]> = (hostname, options, callback) => {
  dns.lookup(hostname, { all: true, verbatim: true }, (error, addresses) => {
    if (error || !addresses.length || addresses.some(item => !isPublicAddress(item.address))) {
      callback(new Error("Attachment host is unavailable or not public."), "", 4); return;
    }
    const all = typeof options === "object" && options.all;
    if (all) (callback as Function)(null, addresses);
    else callback(null, addresses[0].address, addresses[0].family);
  });
};
const agent = new https.Agent({ lookup: publicLookup, keepAlive: false });
export async function downloadAttachment(value: string, request: typeof axios.get = axios.get): Promise<Buffer> {
  ...
  try {
    for (let redirects = 0; redirects <= 3; redirects++) {
      ...
      if ([301, 302, 303, 307, 308].includes(response.status) && response.headers.location) {
        url = attachmentUrl(new URL(response.headers.location, url).href); continue;
      }
      if (response.status !== 200) throw new Error("Attachment download failed.");
      const buffer = Buffer.from(response.data);
      if (!buffer.length || buffer.length > MAX_DOCUMENT_BYTES) throw new Error("Attachment size exceeds limit.");
      return buffer;
    }
...
export function decodeDocument(value: string): Buffer {
  const normalized = value.trim().replace(/^data:[^;,]+;base64,/i, "").replace(/\s/g, "");
  if (!normalized || normalized.length % 4 !== 0 || !/^[A-Za-z0-9+/]*={0,2}$/.test(normalized) || normalized.length > Math.ceil(MAX_DOCUMENT_BYTES / 3) * 4) throw new Error("INVALID_DOCUMENT: file_base64 must contain a valid document of at most 12 MiB.");
  const buffer = Buffer.from(normalized, "base64");
  if (!buffer.length || buffer.length > MAX_DOCUMENT_BYTES || buffer.toString("base64") !== normalized) throw new Error("INVALID_DOCUMENT: Invalid or oversized base64 document.");
  return buffer;
}
export function documentMetadata(buffer: Buffer, filename?: string) {
  ...
  if (buffer.subarray(0, 100).toString().trimStart().match(/^<(?:!doctype\s+html|html)/i)) throw new Error("INVALID_DOCUMENT: The attachment link returned a web page. Supply the original file or resume_text.");
