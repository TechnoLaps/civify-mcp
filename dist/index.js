...
async function generatePdf(args, sessionAuth) {
    ...
    if (bytes.subarray(0, 5).toString() !== "%PDF-" || bytes.length > MAX_DOCUMENT_BYTES)
        throw new Error("INVALID_PDF: Renderer returned an invalid or oversized PDF.");
...
for (const tool of TOOLS) {
    ...
    if (tool.name === "civify_tailor_cv") {
        tool.inputSchema.anyOf.push({ required: ["resume_data"] });
...
export const createMcpServer = (initialSessionAuth) => {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            ...
            if (["civify_score_ats", "civify_tailor_cv"].includes(name) && args?.resume_data && ["file", "file_url", "file_base64", "resume_text", "server_file_path", "file_path"].some(key => args?.[key] !== undefined))
                throw new Error("INVALID_ARGUMENT: Use resume_data alone, or supply one original resume input.");
            const executeTool = async () => {
                switch (name) {
                    ...
                    case "civify_tailor_cv": {
                        ...
                        const client = getApiClient(sessionAuth, apiKey);
                        const form = new FormData();
                        if (!args?.resume_data && !await appendResumeInput(form, args))
                            throw new Error("INVALID_ARGUMENT: Provide resume_data or one original resume input.");
                        ...
                        if (res.data?.success === false)
                            return { content: [{ type: "text", text: JSON.stringify(res.data) }], isError: true };
                        const tailored = res.data?.tailoredCv;
                        if (!tailored || typeof tailored !== "object")
                            throw new Error("INCOMPLETE_RESULT: Tailoring returned no resume. Contact support using the request ID; do not automatically repeat this paid operation.");
                        ...
                        if (args?.export_pdf !== false) {
                            ...
                            data.document = { status: "NOT_REQUESTED", next_tool: "civify_generate_pdf" };
            ...
            let data = result.structuredContent?.data ?? null;
