...
export class CivifyOAuthProvider {
    ...
    transaction = (req, res) => {
        ...
        if (!pending || pending.expires <= Date.now()) {
            res.status(404).json({ error: "Authorization expired. Reconnect from your client." });
            return;
        }
    ...
    complete = async (req, res) => {
        ...
        let apiKey;
        let authorizationExpires;
        try {
            if (typeof req.query.code !== "string" || !/^[A-Za-z0-9_-]{43}$/.test(req.query.code))
                throw new Error("Invalid code");
            const exchanged = await axios.post(`${this.apiUrl}/mcp/account-link/exchange`, { code: req.query.code, verifier: pending.verifier, transaction: id }, { timeout: 15000, maxRedirects: 0 });
            apiKey = exchanged.data.apiKey;
            if (typeof apiKey !== "string" || !apiKey.startsWith("cv-fy-"))
                throw new Error("Invalid grant");
            authorizationExpires = exchanged.data.expiresAt;
            if (!Number.isFinite(authorizationExpires) || authorizationExpires <= Date.now())
                throw new Error("Expired grant");
        }
        catch {
            // Never log the upstream response: it may contain credentials.
            res.status(400).send("Unable to complete Civify account linking. Restart from your MCP client and sign in again.");
            return;
        }
        const code = opaque();
    ...
    issue(clientId, apiKey, family, authorizationExpires = Date.now() + 30 * 86400_000) {
        if (authorizationExpires <= Date.now())
            throw new InvalidGrantError("Account connection expired; reconnect Civify.");
